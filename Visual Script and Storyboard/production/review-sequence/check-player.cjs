...
(async () => {
  ...
  arrow('scene_info', 'lower-left');
  assert(node('scene-info-panel').classes.has('lower-left'), 'Moves during playback');
  ...
  assert(node('scene-context-panel').classes.has('upper-left'), 'Corner collision swaps only current clip');
  node('previous').click(); await settle();
  assert(node('scene-info-panel').classes.has('lower-left')); assert(p.audio.muted);
  ...
  await p.flush();
  assert.equal(p.server.clips[0].panel_positions.scene_info, 'lower-left'); assert(p.server.clips[0].muted);
  ...
  assert(reopened.nodes.get('scene-info-panel').classes.has('lower-left')); assert(reopened.audio.muted);
